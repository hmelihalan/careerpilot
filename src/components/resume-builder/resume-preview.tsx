...
function Section({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="mt-[18px]">
      <h2 className="border-b border-slate-700 pb-1 text-[11.5px] font-bold uppercase tracking-[0.12em] text-slate-950">
        {title}
      </h2>
...
export function ResumePreview({ draft }: { draft: ResumeDocument }) {
  ...
  return (
    <article
      ...
      className="resume-print-document mx-auto aspect-[210/297] w-full max-w-190 bg-white px-[7.75%] py-[7.75%] font-sans text-[13px] leading-[1.36] text-slate-800 shadow-sm ring-1 ring-slate-200"
      aria-label="Resume preview"
    >
      <header className="text-center">
        <h1 className="text-[26px] font-bold leading-tight tracking-[0.025em] text-slate-950">
        ...
        {contact.length ? (
          <p className="mt-2 flex flex-wrap justify-center gap-x-2 text-[11px] text-slate-600">
        ...
        {links.length ? (
          <p className="mt-1 flex flex-wrap justify-center gap-x-3 text-[11px] text-slate-600">
            {links.map((item) => (
          ...
          <div className="space-y-1">
            {draft.certifications.map((item) => (
              <div key={item.id} className="flex justify-between gap-4">
                ...
                <p className="shrink-0 text-[11px] text-slate-600">{item.date}</p>
              </div>
            ))}
          </div>
        </Section>
