...
export function PipelinePreview({ applicationsPath }: PipelinePreviewProps) {
  return (
    <Card size="sm" className="border border-slate-200 shadow-none ring-0">
      <CardHeader className="flex-row items-center justify-between">
        <div>
          <CardTitle>Application Pipeline</CardTitle>
          <p className="mt-0.5 text-xs text-slate-500">Track your active opportunities.</p>
        </div>
        <Link
      ...
      <CardContent>
        <div className="scrollbar-thin overflow-x-auto pb-1">
          <div className="grid grid-cols-[repeat(5,minmax(120px,1fr))] gap-2.5">
            {pipeline.map((column) => (
              <section key={column.stage} aria-labelledby={`pipeline-${column.stage}`} className="min-w-0">
                      ...
                      <article className="min-w-0">
                        ...
                        <div className="mt-2 flex items-center justify-between gap-1.5">
                          <span className="flex min-w-0 items-center gap-1 text-[10px] text-slate-500">
                            ...
                            <span className="truncate">{application.workMode}</span>
                          </span>
